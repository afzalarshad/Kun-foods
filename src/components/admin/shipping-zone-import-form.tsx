...
export function ShippingZoneImportForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/import/shipping-zones", { method: "POST", body: formData });
  ...
  return (
    <div>
      <form ref={formRef} onSubmit={handleSubmit} className="flex flex-wrap items-center gap-3">
        ...
        >
          {isSubmitting ? "Importing…" : "Import shipping rates"}
        </button>
      </form>
